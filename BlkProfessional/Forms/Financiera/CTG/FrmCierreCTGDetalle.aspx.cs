...
{
    public partial class FrmCierreCTGDetalle : System.Web.UI.Page
    {
        ...
        protected void gvCierre_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Visualizar")
            {
                string idCierre = Request.QueryString["IdCierre"];
                string usuario = Request.QueryString["usuario"];
                string terminal = e.CommandArgument.ToString();
                Response.Redirect($"FrmCierreCTGAlm.aspx?usuario={usuario}&IdCierre={idCierre}&Terminal={terminal}");
            }
        }
        ...
        protected void btnResumen_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dtb = BRL.GFCierreFinancieroDetalle_BRL.SelectTable(objeto, 14);
                if (dtb.Rows.Count > 0)
                {
        ...
        protected void gvCierre_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                decimal TotalValor = Convert.ToDecimal(e.Row.Cells[2].Text);
                e.Row.Cells[2].Text = String.Format("${0:#,##0}", TotalValor);
        ...
        protected void btnVolver_Click(object sender, EventArgs e)
        {
            ...
            string usuario = Request.QueryString["usuario"];
            string idCierre = Request.QueryString["IdCierre"];
            Response.Redirect($"FrmCierreCTG.aspx?usuario={usuario}&IdCierre={idCierre}");
        }
    }
}